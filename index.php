...
if($posCtr===false){
	...
	$version = '?v=09';
    $loginAccepted = isset($_SESSION['chamilo_h5p']) ? $_SESSION['chamilo_h5p_accepted'] : null;
    
	$pwp = api_get_path(WEB_PLUGIN_PATH);
	if($ctrAction=='edit_item'||$ctrAction=='add_item'){
		$fh .= '<script src="'.$pwp.'chamilo_h5p/resources/js/h5p_extras.js'.$version.'" type="text/javascript" ></script>';
        ...
        $fh .= '<link rel="stylesheet" type="text/css" href="'.$pwp.'chamilo_h5p/resources/css/window-h5p.css">';
